...
class MLPArchitectures:
    ...
    def create_classifier(
        ...
        for size in hidden_sizes:
            layers.append(
                LayerConfig(
                    size=size, activation=hidden_activation, dropout_rate=dropout_rate
                )
            )
    ...
    @staticmethod
    def create_wide_and_deep(
        input_size: int, wide_size: int, deep_layers: List[int], output_size: int
    ) -> List[LayerConfig]:
        ...
        for size in deep_layers:
            layers.append(LayerConfig(size=size, activation="relu", dropout_rate=0.2))
    ...
    def create_residual_mlp(
        input_size: int, hidden_size: int, num_residual_blocks: int, output_size: int
    ) -> List[LayerConfig]:
        ...
        for _ in range(num_residual_blocks):
            layers.extend(
                [LayerConfig(hidden_size, "relu"), LayerConfig(hidden_size, "relu")]
            )
