...
class AutoencoderArchitectures:
    ...
    def create_variational_autoencoder(
        input_size: int, encoding_sizes: List[int], latent_dim: int
    ) -> List[LayerConfig]:
    ...
    def create_denoising_autoencoder(
        input_size: int, encoding_sizes: List[int], noise_level: float = 0.1
    ) -> List[LayerConfig]:
    ...
    def create_sparse_autoencoder(
        input_size: int, encoding_sizes: List[int], sparsity_level: float = 0.05
    ) -> List[LayerConfig]:
        ...
        for size in encoding_sizes:
            layers.append(
                LayerConfig(
                    ...
                    activation="relu",
                    dropout_rate=1.0
                    - sparsity_level,  # Enforce sparsity through dropout
                )
            )
