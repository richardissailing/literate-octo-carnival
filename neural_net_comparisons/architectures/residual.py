...
class ResidualArchitectures:
    ...
    def create_resnet_block(
        input_size: int, hidden_size: Optional[int] = None, activation: str = "relu"
    ) -> List[LayerConfig]:
        ...
        layers.append(
            LayerConfig(input_size, "linear")
        )  # Linear activation before residual addition

        return layers

    @staticmethod
    def create_residual_network(
        input_size: int, hidden_size: int, num_blocks: int, output_size: int
    ) -> List[LayerConfig]:
    ...
    @staticmethod
    def create_dense_residual_network(
        input_size: int, growth_rate: int, num_blocks: int, output_size: int
    ) -> List[LayerConfig]:
    ...
    @staticmethod
    def create_pyramid_residual_network(
        input_size: int,
        ...
        for stage in range(num_stages):
            ...
            for block in range(blocks_per_stage):
                layers.extend(
                    ResidualArchitectures.create_resnet_block(
