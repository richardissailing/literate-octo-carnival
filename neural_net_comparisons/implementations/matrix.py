...
class Matrix:
    ...
    def _multiply_chunk(
        self, other: "Matrix", start_row: int, end_row: int, result: "Matrix"
    ):
        """Process a chunk of matrix multiplication using NumPy"""
        result.data[start_row:end_row] = np.dot(
            self.data[start_row:end_row], other.data
        )
    ...
    def multiply(self, other: "Matrix", num_threads: int = 4) -> "Matrix":
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            for start in range(0, self.rows, chunk_size):
                ...
                futures.append(
                    executor.submit(
                        self._multiply_chunk, other, start, end, result
                    )
                )
    ...
    def multiply_elementwise(self, other: "Matrix") -> "Matrix":
        ...
        if self.rows != other.rows or self.cols != other.cols:
            raise ValueError(
                "Invalid dimensions for elementwise multiplication"
            )
