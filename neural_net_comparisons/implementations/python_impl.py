...
class Layer:
    def __init__(self, config: LayerConfig):
        ...
        self.activation_func = ActivationFunctions.get_activation_pair(
            config.activation
        )
...
class EnhancedNeuralNetwork:
    ...
    def _initialize_weights(self):
        ...
        for i in range(len(self.layers) - 1):
            ...
            weights.data = np.random.randn(
                current_layer.size, next_layer.size) * scale
        ...
        for i, layer in enumerate(self.layers[:-1]):
            ...
            if layer.bias is not None:
                ...
                output = output.add(
                    Matrix.from_numpy(
                        np.broadcast_to(
                            layer.bias.data, (output.rows, layer.bias.cols)
                        )
                    )
                )
    ...
    def backward(self, X: Matrix, y: Matrix, learning_rate: float) -> float:
        ...
        for i in range(len(self.layers) - 2, -1, -1):
            ...
            weight_gradients = input_transpose.multiply(
                error, self.num_threads
            )
